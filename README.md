...
Enter build environment:
 - Linux: `docker run --rm -it -v --privileged "$(pwd)":/root/env myos-buildenv`
 - Windows (CMD): `docker run --rm -it -v --privileged "%cd%":/root/env myos-buildenv`
 - Windows (PowerShell): `docker run --rm -it -v --privileged "${pwd}:/root/env" myos-buildenv`
 - Please use the linux command if you are using `WSL`, `msys2` or `git bash`
 - NOTE: If you are having trouble with an unshared drive, ensure your docker daemon has access to the drive your development environment is on. For Docker Desktop, this is in "Settings > Shared Drives" or "Settings > Resources > File Sharing".
